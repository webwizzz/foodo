  ...
  return (
    <section
      className={cn(
        ...
        "rounded-[28px] p-8 md:p-10 lg:p-12 flex flex-col font-matter relative",
    ...
    >
      ...
      <div className="lg:text-center text-left">
        <h3 className="font-matter text-xl md:text-3xl lg:text-3xl font-semibold">{title}</h3>
        <div className="mt-3 relative">
          <p 
            className={cn(
              "font-matter font-bold tracking-tight text-3xl md:text-3xl lg:text-5xl transition-all duration-300",
          ...
          {!isPriceVisible && (
            <div className="absolute inset-0 flex items-center justify-center">
              <span className="text-xl md:text-3xl font-semibold opacity-80">••••</span>
            </div>
          )}
        </div>
      </div>

      {/* CTA and Expand Button Row */}
      <div className="mt-7 md:mt-8 flex items-center gap-3">
        <button
          type="button"
          onClick={handleButtonClick}
          className={cn(
            "h-8 md:h-10 flex-1 px-6 md:px-7 rounded-full text-sm md:text-base font-medium transition-colors",
            customBgColor 
        ...
        <div className="md:hidden">
          <button
            ...
            className={cn(
              "w-8 h-8 md:w-12 md:h-12 rounded-full flex items-center justify-center transition-all duration-300",
              customBgColor 
                ? (isLightBg ? "bg-black/10 hover:bg-black/20" : "bg-white/10 hover:bg-white/20")
                : (isHighlight ? "bg-black/10 hover:bg-black/20" : "bg-white/10 hover:bg-white/20"),
            )}
      ...
      {/* Features section - always visible on desktop, collapsible on mobile */}
        ...
        <AnimatePresence>
          {isExpanded && (
            ...
            >
              ...
              >
                {base.map((feature, i) => (
                  <li key={`base-mobile-${i}`} className="pt-1">
                    {feature}
                  </li>
                ))}
              </ul>
                  ...
                  {groups.map((group, idx) => (
                    <div key={group.title + idx}>
                      ...
                      <ul className="space-y-3">
                        {group.items.map((item, i) => (
                          <li key={`item-mobile-${idx}-${i}`} className="flex items-start gap-3">
                            <CheckCircle2
                              className={cn(
                                "shrink-0 mt-0.5", 
                                customBgColor 
